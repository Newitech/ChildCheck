...
# Bun treats the lockfile as frozen in CI-like environments. When the lockfile
# was generated by a different Bun version, bun install/run throws
# "lockfile had changes, but lockfile is frozen". Deleting the lockfile before
# any bun command forces a fresh resolution — no lockfile = no frozen check.
# We also use --no-install on `bun run` to prevent it from auto-installing.
rm -f bun.lock
